...
|---|---|---|---|
| `aarch64` | aarch64-unknown-none-elf-gcc 14+, GNU MIG (master), binutils 2.46 | yes | yes - kernel boots, all 11 kernel tests pass under `qemu-system-aarch64 -M virt` via the guest-loader test harness |
| `x86_64` | x86_64-unknown-elf-gcc | yes | yes - all kernel tests pass under `qemu-system-x86_64 + KVM` on a Linux/x86_64 host |
| `x86_64-xen` | x86_64-unknown-elf-gcc | yes | n/a - gnumach's `tests/Makefrag.am` disables the suite when `--enable-platform=xen` |
| `i686` | i686-unknown-elf-gcc | yes | yes - all kernel tests pass under `qemu-system-i386 + KVM` on a Linux/x86_64 host |
| `i686-xen` | i686-unknown-elf-gcc | yes | n/a - same Xen-block guard as `x86_64-xen` |
...
invocation under [Onboarding section 3](#3-run-aarch64-only-for-now) uses these
deliberately:
