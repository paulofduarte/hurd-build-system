...
#       buildable stand-in).
# Two dev-only sets are then SUBTRACTED, both churn-prone and unreferenced by any
...
push-cache:
	...
	: "FINAL stage drops anything cache.nixos.org already serves - the stock nixpkgs build-deps the deriver-filter keeps (stdenv-linux et al.) plus the content-addressed setup-hooks in their closures (audit-tmpdir.sh leaked in purely via cachix re-expanding stdenv's closure).  Per-path probe because the batch path-info forms misreport presence; our custom artifacts (cross-gcc/glibc/buildtree/binutils + the headless-qemu and deterministic-texinfo overrides) are NOT on cache.nixos.org so they stay - giving the SAME custom-only set on every host.  Fallback: if cache.nixos.org is unreachable every probe fails and the full set is pushed (today's behaviour)."; \
