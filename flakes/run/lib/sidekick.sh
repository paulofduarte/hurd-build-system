...
_sidekick_op() {
  local work="$1" op="$2"; shift 2
  cp "$op" "$work/run.sh"
  local rc; rc=$(sk_oneshot "$work" "$@")
  if [ "${rc:-1}" -ne 0 ]; then
    echo "sidekick: op $(basename "$op") failed (rc=$rc)" >&2
    [ -f "$work/run.out" ] && { echo "--- run.out ---" >&2; cat "$work/run.out" >&2; }
    [ -f "$work/.qemu.log" ] && { echo "--- qemu.log (tail) ---" >&2; tail -20 "$work/.qemu.log" >&2; }
  fi
  return "${rc:-1}"
}
...
sidekick_make_iso() {
  ...
  local work; work="$(dirname "$out_iso")/iso-build"
  local stamp="$out_iso.stamp"
  local cache_hash; cache_hash=$(printf '%s\n%s' "$cache_key" "$grub_cfg" | sha256_stdin)
  ...
  _sidekick_op "$work" "$_SK_OPLIB/sidekick-mkiso.sh" || { rm -rf "$work"; return 1; }
  [ -s "$work/out.iso" ] || die "sidekick mkiso failed: $work/out.iso missing/empty"
  mv "$work/out.iso" "$out_iso"; rm -rf "$work"
