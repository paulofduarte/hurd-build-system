...
{ nixpkgs, system, targets, gcc-src, ownBinutils }:
...
let
  ...
  gccDeps = with pkgs; [ gmp mpfr libmpc isl zlib ];
  ...
  gccHardening = [ "format" "stackclashprotection" ];
  ...
  mkGcc = { name, target, pname, desc, extraConfigure, buildList, installList
          , glibcDrv ? null, extraNativeBuildInputs ? [ ] }:
    let
      ...
      targetMaps = "-ffile-prefix-map=$NIX_BUILD_TOP=${buildFlags.gccCanonRoot}"
        + lib.optionalString (glibcDrv != null)
  ...
  mkBootstrap = name: target: mkGcc {
    inherit name target;
    pname = "bootstrap-gcc-${target.crossTarget}";
    desc  = "Bootstrap (libc-free) cross GCC";
    buildList   = [ "all-gcc" "all-target-libgcc" ];
    installList = [ "install-gcc" "install-target-libgcc" ];
    extraConfigure = [
      "--enable-languages=c"
  ...
  mkFull = name: target: glibcDrv: mkGcc {
...
in
{
  ...
  bootstrap = lib.mapAttrs' (name: target:
    lib.nameValuePair "bootstrap-gcc-${name}" (mkBootstrap name target)) hurdTargets;
