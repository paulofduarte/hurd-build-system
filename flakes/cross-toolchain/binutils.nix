...
{ nixpkgs, system, targets, binutils-src }:
...
let
  ...
  mkOne = name: target:
    ...
    pkgs.stdenv.mkDerivation ({
      ...
      nativeBuildInputs = (with pkgs; [ bison flex texinfo perl gettext ])
...
lib.mapAttrs' (name: target:
  lib.nameValuePair "cross-binutils-${name}" (mkOne name target)) targets
