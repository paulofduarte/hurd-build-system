...
#
# The VM is booted on first use (one per host, lock-guarded), reused warm via an
# SSH ControlMaster, and auto-stopped by an idle reaper. Nothing host-specific is
# baked into the guest: the project path and a dedicated throwaway pubkey are
# injected on the kernel cmdline at boot. See flakes/sidekick/guest.nix.
#
...
IDLE_SECS=600     # auto-shutdown after this many seconds idle
...
[ "$#" -ge 1 ] || die "usage: sidekick-run <tool> [args...]"
...
REAPPID="$CTL/reaper.pid"
KEY="$CTL/id_ed25519"
MUX="$CTL/ssh-mux"
LASTUSE="$CTL/last-use"
...
start_reaper() {
  # One detached watcher: stop the VM after IDLE_SECS with no calls.
  [ -f "$REAPPID" ] && kill -0 "$(cat "$REAPPID")" 2>/dev/null && return 0
  (
    while sleep 60; do
      [ -f "$PIDFILE" ] && kill -0 "$(cat "$PIDFILE")" 2>/dev/null || exit 0
      last=$(stat -f %m "$LASTUSE" 2>/dev/null || echo 0)
      now=$(date +%s)
      if [ "$((now - last))" -gt "$IDLE_SECS" ]; then
        rest_stop
        sleep 5
        kill "$(cat "$PIDFILE")" 2>/dev/null || true
        rm -f "$PIDFILE" "$SSH_SOCK" "$CTL_SOCK"
        exit 0
      fi
    done
  ) >/dev/null 2>&1 &
...
boot_vm() {
  ...
  for _ in $(seq 1 80); do
    "$SSH" "${ssh_opts[@]}" root@sidekick true 2>/dev/null && break
    sleep 0.5
  done
  "$SSH" "${ssh_opts[@]}" root@sidekick true 2>/dev/null || {
    tail -30 "$BOOTLOG" >&2
...
exec "$SSH" "${ssh_opts[@]}" root@sidekick \
