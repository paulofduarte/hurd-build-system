...
sk_serve_start() {  # $1 ctldir  $2 keepalive(s)
  local ctl="$1" ka="${2:-60}"
  mkdir -p "$ctl/q"; printf '%s' "$ka" > "$ctl/keepalive"; rm -f "$ctl/.ready" "$ctl/.seq"
  ...
  while [ ! -e "$ctl/.ready" ]; do
    ...
    kill -0 "$(cat "$ctl/.qpid" 2>/dev/null)" 2>/dev/null \
      || { echo "sidekick: serve VM exited before ready" >&2; cat "$ctl/.qemu.log" >&2; return 1; }
    [ "$t" -gt 900 ] && { echo "sidekick: serve VM ready timeout" >&2; cat "$ctl/.qemu.log" >&2; return 1; }
  done
...
sk_send() {  # $1 ctldir ; rest: argv — run in VM, relay stdout/stderr, return its rc
  ...
  while [ ! -e "$ctl/q/$seq.done" ]; do
    sleep 0.1
    kill -0 "$(cat "$ctl/.qpid" 2>/dev/null)" 2>/dev/null \
      || { echo "sidekick: serve VM died mid-request" >&2; return 125; }
  done
  [ -f "$ctl/q/$seq.out" ] && cat "$ctl/q/$seq.out"
  [ -f "$ctl/q/$seq.err" ] && cat "$ctl/q/$seq.err" >&2
  local rc; rc=$(cat "$ctl/q/$seq.rc" 2>/dev/null || echo 125)
